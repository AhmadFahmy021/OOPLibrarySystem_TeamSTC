...
public class BorrowBookPage extends BorderPane {

    private book bookToDisplayAndBorrow;
    ...
    public void displayBook(book book) {
        this.bookToDisplayAndBorrow = book;
        if (book != null) {
            titleLabel.setText(book.getTitle());
            isbnLabel.setText("ISBN : " + (book.getISBN() != null ? book.getISBN() : (book.getId() != null ? book.getId() : "N/A")));
            authorLabel.setText("Author : " + book.getAuthor());
            quantityLabel.setText("Quantity : " + book.getStock());
    ...
    private void handleSubmit() {
        if (bookToDisplayAndBorrow == null) {
        ...
        if (bookToDisplayAndBorrow.getStock() <= 0) {
            showAlert(AlertType.ERROR, "Error Peminjaman", "Maaf, buku ini tidak tersedia.");
            return;
        }

        System.out.println("Meminjam buku: " + bookToDisplayAndBorrow.getTitle());
        System.out.println("Durasi: " + duration + " hari");
        System.out.println("Tanggal Kembali: " + returnDate);

        bookToDisplayAndBorrow.setStock(bookToDisplayAndBorrow.getStock() - 1);
        displayBook(bookToDisplayAndBorrow); 

        showAlert(AlertType.INFORMATION, "Sukses", "Peminjaman buku '" + bookToDisplayAndBorrow.getTitle() + "' berhasil!");
